...
def test_oscillator():
    ...
    my_simulator = simulator(
        my_model_numeric, timesteps_solver=np.linspace(0, 10, NUM_TIMESTEPS_SOLVER)
    )
    my_tdsa = time_dependent_sensitivity_analysis(
        my_simulator, my_distribution, num_timesteps_quadrature=num_timesteps_quadrature
    )
    ...
    assert np.all(
        relative_error <= 0.20
    ), "Relative error between KL and PCE Sobol' indices is higher than 20%"
    assert np.all(
        sobol_indices_KL >= 0
